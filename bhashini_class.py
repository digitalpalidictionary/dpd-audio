...
class Bashini:
    ...
    output_dir = Path("resources/dpd_audio/mp3s")
    ...
    def translit_with_indic_transliteration(self, text_roman: str) -> str:
        """
        Convert Roman script to Kannada using indic_transliteration library.
        """

        try:
            # Convert from IAST to Kannada
            return sanscript.transliterate(
                text_roman, sanscript.IAST, sanscript.KANNADA
            )
        except Exception as e:
            pr.error(f"Error in indic_transliteration: {e}")
            return ""
